...
export const useWebSocket = (
  ...
  const initConnection = useCallback(() => {
    ...
    if (ws.current) {
      console.log('🧹 Cleaning up old socket, state:', ws.current.readyState);
      isIntentionalClose.current = true;
      
      // Remove old event listeners to prevent loops
    ...
    try {
      ...
      socket.onopen = () => {
        ...
        if (subscribedMatchIdsRef.current.size > 0) {
          const matchIds = Array.from(subscribedMatchIdsRef.current);
          console.log('🔄 Re-subscribing to matches:', matchIds);
      ...
      socket.onclose = (event) => {
        ...
        // Clear the ref since socket is closed
  ...
  const connectGlobal = useCallback(() => {
    console.log('🌍 connectGlobal called');
  ...
  const subscribeMatch = useCallback((matchId: string | number) => {
    ...
    console.log('📌 Subscribing to match:', normalized);
  ...
  const unsubscribeMatch = useCallback((matchId: string | number) => {
    const normalized = normalizeId(matchId);
    console.log('📌 Unsubscribing from match:', normalized);
