...
export const useMatchData = (): UseMatchData => {
  ...
  useEffect(() => {
    console.log('🔄 Commentary state updated:', {
  ...
  const handleWSMessage = useCallback((msg: WSMessage) => {
    ...
    switch (msg.type) {
      ...
      // }
        ...
        case "commentary": {
        console.log('📣 COMMENTARY CASE TRIGGERED');
        console.log('msg.data:', msg.data);
        console.log('latestMatchIdRef.current:', latestMatchIdRef.current);
        console.log('msg.data.matchId:', msg.data.matchId);
        
        if (
          latestMatchIdRef.current == null ||
          msg.data.matchId != latestMatchIdRef.current
        ) {
          console.log('❌ COMMENTARY FILTERED OUT');
          console.log('Reason:', latestMatchIdRef.current == null ? 'latestMatchIdRef is null' : 'matchId mismatch');
          return;
        }
        ...
        setCommentary((prev) => {
          console.log('Previous commentary:', prev.length);
          console.log('New commentary:', prev.length + 1);
  ...
  const watchMatch = useCallback(
    (id: string | number) => {
      ...
      setWsError(null);
      latestMatchIdRef.current = numId;
      ...
      if (activeMatchId != null && activeMatchId != numId) {
        unsubscribeMatch(activeMatchId);
      }
      setActiveMatchId(numId);
      subscribedMatchIdsRef.current.add(String(numId));
      subscribeMatch(numId);
      subscribeMatch(id);
